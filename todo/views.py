        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todo/signupuser.html',
                              {'form': UserCreationForm(), 'error': "Username is already in use. Enter new username!!!"})
...
def logoutuser(request):
    if request.method == 'POST': #because if we keep get, browser will load it as a <a> tag and logout me at starting
        logout(request)
